...
$$
\tilde{v}_t(x_t) = v_t(x_t) + \sum_{n=1} \kappa_n(x_t, r) / n!
$$
...
$$
\rho_{k+1} = \arg\min_{\rho} \left\{ J(\rho) + \frac{1}{2\tau} W_2^2(\rho, \rho_k) \right\}
$$
...
$$
\max\left\{ \partial_tV + \mathcal{L}V + f,\; \sup_{\xi} [V(x+\xi) - V(x) - c(\xi)] - V \right\} = 0
$$
